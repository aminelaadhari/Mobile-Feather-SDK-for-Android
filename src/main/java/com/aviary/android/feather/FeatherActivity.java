...
public class FeatherActivity extends MonitoredActivity
        implements OnToolbarClickListener, OnImageDownloadListener, OnToolListener,
    ...
    static {
        logger = LoggerFactory.getLogger(FeatherActivity.class.getSimpleName(),
    ...
    protected void onInitializeUtils() {
        ...
        try {
            ...
            Toast.makeText(getApplicationContext(), "Sorry an error occurred: " + e.getMessage(),
        ...
        switch (id) {
            ...
            case ALERT_DOWNLOAD_ERROR:
                dialog = new AlertDialog.Builder(this).setTitle(R.string.feather_attention)
                        .setMessage(
                                R.string.feather_error_download_image_message).create();
                break;

            case ALERT_REVERT_IMAGE:
                dialog = new AlertDialog.Builder(this).setTitle(
                        R.string.feather_revert_dialog_title)
        ...
        if (intent != null && intent.getData() != null) {
            ...
            if (extras != null) {
                ...
                if (extras.containsKey(Constants.EXTRA_HIDE_EXIT_UNSAVE_CONFIRMATION)) {
                    mHideExitAlertConfirmation = extras.getBoolean(
    ...
    private List<String> loadStandaloneTools() {
        if (PackageManagerUtils.isStandalone(this)) {
            ...
            try {
                ...
                Object instance = ReflectionUtils.invokeStaticMethod(
                        "com.aviary.android.feather.utils.SettingsUtils",
                        "getInstance",
    ...
    private List<ToolEntry> loadTools() {
        ...
        AbstractPanelLoaderService service = mMainController.getService(
                AbstractPanelLoaderService.class);
        if (service == null) {
            return null;
        }
        if (mToolList == null) {
    ...
    protected void onToolsLoaded(List<ToolEntry> listEntries) {
        ...
        mToolsList.setAdapter(new ListAdapter(this, mListEntries,
                dataService.getIntentContainsKey(Constants.EXTRA_WHITELABEL)));
    ...
    public void onDoneClick() {
        ...
        if (null != mMainController) {
            if (mMainController.getEnabled()) {
                ...
                final boolean saveWithNoChanges = dataService.getIntentValue(
     ...
     * @param sizes  int array containing the width and height of the loaded bitmap
     */
    ...
    public void onPreviewChange(Bitmap bitmap, boolean reset) {
        ...
        if (!reset) {
            changed = BitmapUtils.compareBySize(
                    ((IBitmapDrawable) mImageView.getDrawable()).getBitmap(), bitmap);
        }
    ...
    protected void performSave(final Bitmap bitmap, final boolean changed) {
        ...
        if (myExtras != null && myExtras.getBoolean(Constants.EXTRA_RETURN_DATA)) {
            ...
            onSetResult(RESULT_OK,
                    new Intent().setData(service.getDestImageUri()).setAction("inline-data")
                            .putExtras(extras));
            finish();

        } else {
            ThreadUtils.startBackgroundJob(this, null, getString(R.string.feather_save_progress),
                    new Runnable() {
     ...
     *
     * @param bitmap  the current Bitmap handled by the {@link AviaryMainController}
     * @param changed bitmap has been changed by the user
     */
    protected void doSave(Bitmap bitmap, boolean changed) {
        ...
        if (saveUri != null) {
            ...
            String url = Media.insertImage(getContentResolver(), bitmap, "title",
                    "modified with Aviary Feather");
            if (url != null) {
        ...
        if (null != data) {
            ...
            if (null != bundle) {
                try {
                    ...
                    int imageWidth = newexif
                            .getAttributeInt(ExifInterfaceWrapper.TAG_IMAGE_WIDTH, 0);
                    int imageLength = newexif
                            .getAttributeInt(ExifInterfaceWrapper.TAG_IMAGE_LENGTH, 0);

                    // copy all the tags from the original exif
                    // to the new exif
                    newexif.copyFrom(bundle);

                    newexif.setAttribute(ExifInterfaceWrapper.TAG_IMAGE_WIDTH,
                            String.valueOf(imageWidth));
                    newexif.setAttribute(ExifInterfaceWrapper.TAG_IMAGE_LENGTH,
                    ...
                    newexif.setAttribute(ExifInterfaceWrapper.TAG_SOFTWARE,
                            "Aviary for Android " + SDK_VERSION);
    ...
    public void onStart() {
        super.onStart();
        mOrientation = getResources()
                .getConfiguration().orientation; // getWindowManager().getDefaultDisplay().getRotation();
    }
            ...
            if (null == convertView) {
                if (type == TYPE_TOOL) {
                    ...
                    convertView = mInflater
                            .inflate(R.layout.aviary_tool_feedback_layout, parent, false);
                }
